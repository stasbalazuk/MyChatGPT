...
type
  ...
  public
    ...
    constructor Create(const Text, &Type: string; const Param: string = '';
      Code: Int64 = -1); reintroduce;
  end;
...
{$WARNINGS OFF}
  ...
  protected
    ...
    function Get(const Path: string; Response: TStringStream): Integer;
      overload;
    function Delete(const Path: string; Response: TStringStream)
      : Integer; overload;
    function Post(const Path: string; Response: TStringStream)
      : Integer; overload;
    function Post(const Path: string; Body: TJSONObject;
      Response: TStringStream): Integer; overload;
    function Post(const Path: string; Body: TMultipartFormData;
      Response: TStringStream): Integer; overload;
    function ParseResponse<T: class, constructor>(const Code: Int64;
  ...
  public
    ...
    function Delete<TResult: class, constructor>(const Path: string)
      : TResult; overload;
    ...
    function Post<TResult: class, constructor>(const Path: string)
      : TResult; overload;
    function PostForm<TResult: class, constructor;
    TParams: TMultipartFormData, constructor>(const Path: string;
...
function TOpenAIAPI.Post(const Path: string; Body: TJSONObject;
...
begin
  CheckAPI;
  Headers := GetHeaders + [TNetHeader.Create('Content-Type',
    'application/json')];
  Stream := TStringStream.Create;
  try
    Stream.WriteString(Body.ToJSON);
    Stream.Position := 0;
    Result := FHTTPClient.Post(FBaseUrl + '/' + Path, Stream, Response, Headers)
      .StatusCode;
  finally
    Stream.Free;
  end;
...
begin
  CheckAPI;
  Headers := GetHeaders;
  Result := FHTTPClient.Get(FBaseUrl + '/' + Path, Response, Headers)
    .StatusCode;
end;

function TOpenAIAPI.Post(const Path: string; Body: TMultipartFormData;
  Response: TStringStream): Integer;
var
  Headers: TNetHeaders;
begin
  CheckAPI;
  Headers := GetHeaders;
  Result := FHTTPClient.Post(FBaseUrl + '/' + Path, Body, Response, Headers)
    .StatusCode;
end;
...
function TOpenAIAPI.Post<TResult, TParams>(const Path: string;
...
function TOpenAIAPI.Delete(const Path: string; Response: TStringStream)
  : Integer;
var
  Headers: TNetHeaders;
begin
  CheckAPI;
  Headers := GetHeaders;
  Result := FHTTPClient.Delete(FBaseUrl + '/' + Path, Response, Headers)
    .StatusCode;
end;
...
function TOpenAIAPI.PostForm<TResult, TParams>(const Path: string;
  ParamProc: TProc<TParams>): TResult;
var
  Response: TStringStream;
  Params: TParams;
...
begin
  ...
  if not FOrganization.IsEmpty then
    Result := Result + [TNetHeader.Create('OpenAI-Organization',
      FOrganization)];
end;
...
begin
  case Code of
    429:
      raise OpenAIExceptionRateLimitError.Create(Error.Message, Error.&Type,
        Error.Param, Error.Code);
    400, 404, 415:
      raise OpenAIExceptionInvalidRequestError.Create(Error.Message,
        Error.&Type, Error.Param, Error.Code);
    401:
      raise OpenAIExceptionAuthenticationError.Create(Error.Message,
        Error.&Type, Error.Param, Error.Code);
    403:
      raise OpenAIExceptionPermissionError.Create(Error.Message, Error.&Type,
        Error.Param, Error.Code);
    409:
      raise OpenAIExceptionTryAgain.Create(Error.Message, Error.&Type,
        Error.Param, Error.Code);
  else
    raise OpenAIException.Create(Error.Message, Error.&Type, Error.Param,
      Error.Code);
  end;
end;
...
{$ELSE}
      Error := TJson.JsonToObject<TErrorResponse>
        (UTF8ToString(RawByteString(ResponseText)));
{$ENDIF}
    except
      Error := nil;
    end;
...
function TOpenAIAPI.ParseResponse<T>(const Code: Int64;
  const ResponseText: string): T;
begin
  case Code of
    200 .. 299:
...
{$ENDIF}
  ...
  if not Assigned(Result) then
    raise OpenAIExceptionInvalidResponse.Create('Empty or invalid response', '',
      '', Code);
end;
...
constructor OpenAIException.Create(const Text, &Type, Param: string;
  Code: Int64);
begin
